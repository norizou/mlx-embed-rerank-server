...
| **v4.1-Small** | **単一チェックポイントで全部**（クローン + caption + 自動長さ） | 8bit のみ（fp16 は §7/§8 で品質が同等・速度メモリで劣ると判明し削除） |
...
- **音声モデルは自動アンロードされません**。Qwen3-VL のようなアイドルアンロード対象外で、
  一度ロードすると常駐します。0.6B と 1.7B のように複数サイズを叩くと両方が常駐します。
- **ストリーミング非対応**。Irodori 側は `stream=True` が `NotImplementedError` を投げるため、
  サーバーは全チャンクを収集してから 1 レスポンスで返します。
- **`DEFAULT_TTS` は Irodori 8bit**（`irodori-tts-v4.1-small-8bit`）。§7/§8 のベンチマークと
  試聴の結果を踏まえて切り替えました。Irodori には Qwen3 の `voice` プリセットに相当する
  既定話者が無いため、`ref_audio` と `instruct` を両方省略した呼び出しは（無条件生成による
  素性不明の声を返す代わりに）`400` で拒否します。Qwen3 モデルを明示指定する場合はこの制約は
  かかりません。
